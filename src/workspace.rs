...
pub fn configuration(
    ...
    // We ignore our parameters because they are not useful.
    // The "section" parameter is used to identify the language server, but we already know
    // that from the filetype. Since we simply send initialization options, we actually don't
    // want users to put section names in there, because it would make the initialization
    // options unusable for initialization and for didChangeConfiguration.
    // The "scopeUri" parameter gives a file/directory that the config should apply to,
    // but kak-lsp doesn't have a concept of per-file configuration, and workspaces
    // are separated by kak-lsp processes.

    let dynamic_config = request_dynamic_configuration_from_kakoune(&meta, ctx);
    let settings = match dynamic_config
        .and_then(|cfg| cfg.initialization_options)
        .or_else(|| {
            ctx.config
                .language
                .get(&ctx.language_id)
                .and_then(|conf| conf.initialization_options.clone())
        }) {
        Some(settings) => settings,
        None => return Ok(Value::Array(Vec::new())),
    };

    Ok(Value::Array(vec![settings]))
}
